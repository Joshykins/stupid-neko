...
export default function Popup() {
	...
	const [settingsTab, setSettingsTab] =
		React.useState<SettingsTab>("integration");
	...
	React.useEffect(() => {
		try {
			chrome.storage.sync.get(["integrationId", "widgetEnabled"], (items) => {
				const id =
					typeof items?.integrationId === "string" ? items.integrationId : "";
				setIntegrationId(id);
				setHasKey(!!id);
				const we =
					typeof items?.widgetEnabled === "boolean"
						? items.widgetEnabled
						: true;
				setWidgetEnabled(we);
			});
				...
				try {
					chrome.runtime.sendMessage({ type: "REFRESH_AUTH" }, (resp) => {
						...
						if (ok && authed) {
							...
							setSaving(false);
							setSaveError(
								"Invalid Integration ID. Please verify and try again.",
							);
						}
					});
				} catch (e) {
						...
						<AnimatePresence mode="wait">
							{!showEdit ? (
								<motion.div
									...
									className="mt-2 flex flex-col items-center w-full px-2"
								>
											...
											<div className="flex items-center gap-3">
												{auth.me.image ? (
													...
													<img
														...
														className="h-8 w-8 rounded-full border border-neutral-300 object-cover"
													/>
												) : (
													<div className="h-8 w-8 rounded-full border border-neutral-300 bg-neutral-100 flex items-center justify-center text-xs">
														{(auth.me.name || "U").slice(0, 1)}
													</div>
												)}
												<div className="text-left">
													<div className="text-sm font-semibold leading-tight">
														{auth.me.name || "Your Account"}
													</div>
													{auth.me.currentStreak != null && (
														<div className="text-xs text-gray-600 leading-tight">
															Streak: {auth.me.currentStreak} days
														</div>
													)}
												</div>
											</div>
									...
									{settingsTab === "integration" ? (
										<div>
											...
											<p className="mt-1 text-xs text-gray-600">
												Paste your Integration ID from your{" "}
												<a
													href="https://stupidneko.com/account"
													target="_blank"
													rel="noopener noreferrer"
											...
											<div className="mt-3 flex items-center justify-between">
												...
												<Switch
													...
													onCheckedChange={(v) => {
														...
														try {
															chrome.storage.sync.set({ widgetEnabled: next });
														} catch {}
													}}
