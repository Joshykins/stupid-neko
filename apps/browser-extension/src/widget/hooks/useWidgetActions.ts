...
export function useWidgetActions() {
	...
	const defaultDontTrack = useCallback(async () => {
		try {
			await callBackground('WIDGET_ACTION', {
				action: 'website-provider.dont-track',
			});
		} catch (error) {
	...
	const defaultQuestionTrackOnce = useCallback(async () => {
		try {
			await callBackground('WIDGET_ACTION', {
				action: 'website-provider.question-track-once',
			});
		} catch (error) {
	...
	const defaultQuestionAlwaysTrack = useCallback(async () => {
		try {
			await callBackground('WIDGET_ACTION', {
				action: 'website-provider.question-always-track',
			});
		} catch (error) {
	...
	const defaultStopRecording = useCallback(async () => {
		try {
			await callBackground('WIDGET_ACTION', {
				action: 'website-provider.stop-recording',
			});
		} catch (error) {
	...
	const defaultTrackAnyway = useCallback(async () => {
		try {
			await callBackground('WIDGET_ACTION', {
				action: 'website-provider.track-anyway',
			});
		} catch (error) {
