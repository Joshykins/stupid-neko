...
export const YouTubeTrackingVerified: React.FC<
	YouTubeTrackingVerifiedProps
> = ({ widgetState, currentTime, renderDebugInfo }) => {
	...
	const computeSessionSeconds = (): number => {
		...
		const extra =
			widgetState.isPlaying && widgetState.sessionStartedAt
			...
			<div className="">
				...
				<div className="snbex:flex snbex:items-center snbex:gap-2 snbex:mb-3">
					...
					<div className="snbex:text-lg snbex:font-bold">
						Tracking {formatTime(computeSessionSeconds())} of Progress...
					</div>
				</div>

				{/* Playback status */}
				{widgetState.playbackStatus &&
					widgetState.playbackStatus !== 'playing' && (
						<div className="snbex:mb-2 snbex:text-sm snbex:text-background/80">
							{widgetState.playbackStatus === 'paused'
								? 'Video paused — tracking stopped'
								: 'Video ended — tracking stopped'}
						</div>
					)}
