				...
				<div className="flex items-center gap-2">
					...
					<div>
						<p
							...
							title={`Recording ${getProviderName(state.provider || 'default')}`}
						>
							Recording {getProviderName(state.provider || 'default')}
						</p>
						<p className="text-xs text-gray-500">{state.domain}</p>
					</div>
				</div>
				<div className="flex items-center gap-1">
					<div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
					<span className="text-xs text-gray-500" title="Live recording">
						LIVE
					</span>
				</div>
			</div>
