...
export default function TrackingWidget(props: TrackingWidgetProps) {
	...
	const dailyStreak =
		(typeof me?.currentStreak === "number" ? me.currentStreak : 0) || 0;
	...
	function snapToEdges(p: { left: number; top: number }): {
		left: number;
		top: number;
	} {
		const maxLeft = Math.max(H_PAD, window.innerWidth - ICON_PX - H_PAD);
	...
	const IconButton = (
		<motion.div
			...
			onUpdate={(latest) => {
				...
				if (
					typeof (latest as any).x === "number" ||
					typeof (latest as any).y === "number"
				) {
	...
	return (
		<Popover open={expanded} onOpenChange={setExpanded}>
			...
			<PopoverContent className="!p-[16px]" style={{ padding: "16px" }}>
				<div
					className="!flex !items-center !font-display !gap-2"
					style={{ cursor: isDragging ? "grabbing" : "grab" }}
				>
					<div className="!flex !items-center !gap-2 !font-semibold">
						<span className="!text-inherit">Tracking</span>
						<LanguageFlagSVG
							language={props.languageCode || "ja"}
							className="!h-4 !w-6"
						/>
					</div>
					<button
				...
				<div className="!mt-3 !text-[24px] !font-bold !leading-snug">
					Hey <span className="!font-black">{me?.name || userName}</span>!{" "}
					<span className="!opacity-80 !font-semibold">{encouragement}</span>
				</div>
				...
				<div className="!mt-4 !text-[14px] !leading-relaxed">
					<span className="!font-black">{nekos.toLocaleString()}</span> nekos
					watched <span className="!font-semibold !italic">{contentLabel}</span>{" "}
					to learn <span className="!font-black">Japanese</span>! Totaling{" "}
					<span className="!font-black">{hours.toLocaleString()}</span> tracked
					hours. And{" "}
