...
export async function OPTIONS(request: Request) {
    ...
    const allowOrigin = origin && (origin.startsWith("chrome-extension://") || origin.startsWith("http://") || origin.startsWith("https://")) ? origin : "*";
...
export async function GET(request: Request) {
    ...
    if (!isAuthed) {
        return new NextResponse("Unauthorized", { status: 401, headers: corsHeaders(allowOrigin) });
    }
