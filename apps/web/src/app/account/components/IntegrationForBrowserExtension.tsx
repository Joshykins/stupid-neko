...
export default function IntegrationForBrowserExtension() {
    ...
    const isConnected = !!(integrationKey?.integrationId && integrationKey?.integrationKeyUsedByPlugin);
        ...
        <div className="flex items-center justify-between gap-4 p-3 rounded-lg border border-border/50 hover:border-border transition-colors">
            <div className="flex items-center gap-3">
                <div className="flex h-8 w-8 items-center justify-center rounded-md bg-muted/50">
                    <Image src="/brands/browser-extension.svg" alt="Browser Extension" width={20} height={20} className="w-8 h-8" />
                </div>
                <div>
                    <Label className="font-medium">Browser Extension</Label>
                    <div className="text-xs text-muted-foreground">
                        {isConnected ? 'Connected and tracking web activity' : 'Track web activity automatically.'}
                    </div>
                </div>
            </div>
            <Switch
                style={{ '--switch-checked-bg': 'var(--color-source-misc)' } as React.CSSProperties}
                checked={isConnected}
                onCheckedChange={v => {
            ...
            >
                <DialogContent className="max-w-2xl">
                    ...
                    <DialogFooter>
                        <Button variant="neutral" onClick={() => setGuideOpen(false)}>Close</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            >
                <DialogContent className="max-w-2xl">
                    ...
                    <DialogFooter className="flex justify-between">
                        ...
                        <Button variant="neutral" onClick={() => setManageOpen(false)}>Close</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={confirmOpen} onOpenChange={o => setConfirmOpen(o)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            This will stop auto-tracking from the browser extension. You can re-enable it anytime.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
