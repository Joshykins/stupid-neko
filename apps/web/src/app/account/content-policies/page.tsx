					...
					<div className="flex items-center gap-6 pb-4">
						...
						<div>
							...
							<p className="text-sm text-main-foreground/70 mt-1">
								Configured automatic tracking behavior for specific
								contentKeys.Manage &apos;allow&apos; (auto-start) and
								&apos;block&apos; (never track) policies.
							</p>
						</div>
					</div>
					<PoliciesManager />
...
function PoliciesManager() {
	...
	const [policyKind, setPolicyKind] = useState<'allow' | 'block' | undefined>(
		undefined
	);
	const [cursor, setCursor] = useState<string | undefined>(undefined);
	const [items, setItems] = useState<Array<PolicyItem>>([]);
	const [autoLoading, setAutoLoading] = useState(false);
	const [lastAutoCursor, setLastAutoCursor] = useState<string | undefined>(
		undefined
	);
	...
	const data = useQuery(
		api.contentLabelPolicyFunctions.listUserContentLabelPolicies,
		{
	...
	const del = useMutation(
		api.contentLabelPolicyFunctions.deleteUserContentLabelPolicy
	);
	...
	React.useEffect(() => {
		...
		if (autoLoading) {
			if (data.items.length > 0) {
				...
				data.continueCursor &&
				data.continueCursor !== cursor &&
				data.continueCursor !== lastAutoCursor
			) {
								...
								<Dialog>
									...
									<DialogContent>
										<DialogHeader>
											...
											<DialogDescription>
												This action cannot be undone. The policy will be
												removed.
											</DialogDescription>
										</DialogHeader>
										<DialogFooter>
...
function SourceCombobox({
	...
	const isContentSource = (val: string): val is ContentSource =>
		['website', 'youtube', 'spotify', 'anki', 'manual'].includes(
			val as ContentSource
		);
