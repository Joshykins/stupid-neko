...
export function FavoritesAddDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void; }) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl">
                <DialogHeader>
                    <DialogTitle className="flex items-center"><Star className="mr-2 !size-6 fill-yellow-300 stroke-border" /> Add From Favorite</DialogTitle>
                    <DialogDescription>Pick a favorite template to quickly add a record, or manage favorites.</DialogDescription>
                </DialogHeader>
                ...
                <div>
                            ...
                            {!favorites && (
                                <div className="flex items-center justify-center h-[300px]">
                                    <div className="text-center">
                                        ...
                                        <div className="mt-2 text-sm text-muted-foreground">Fetching your favorites…</div>
                                    </div>
                                </div>
                            )}
                            {favorites && favorites.length === 0 && (
                                <div className="flex items-center justify-center h-[300px]">
                                    <div className="text-center">
                                        ...
                                        <div className="mt-2 text-sm text-muted-foreground">No favorites yet. Use previous manual records to add favorites.</div>
                                    </div>
                                </div>
                            )}
                            {favorites && favorites.length > 0 && (
                    ...
                    <div className="mt-3"><Button variant="neutral" size="sm" onClick={() => { setCursor(undefined); setHistoryCursorStack([]); setHistoryOpen(true); }}><History className="mr-1" /> Previous Manual Records</Button></div>
                </div>
                                                ...
                                                <li key={(r as any)._id} className="flex items-center justify-between gap-3 p-2 rounded-base border-2 border-border bg-secondary-background">
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <div className="text-xs text-muted-foreground flex items-center gap-2">
                                                            <span>{Math.max(0, Math.round((((r as any).durationInSeconds ?? 0) / 60)))}m</span>
            ...
            <div className="flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                    {(favorite as any).externalUrl ? (
                        <span className="inline-flex items-center gap-1 max-w-full">
                            <a href={(favorite as any).externalUrl} target="_blank" rel="noreferrer" className="font-bold truncate underline decoration-main hover:text-main-foreground/80">{title || "(untitled)"}</a>
                            <a href={(favorite as any).externalUrl} target="_blank" rel="noreferrer" aria-label="Open link" className="text-main-foreground/80 hover:text-main-foreground flex-shrink-0"><ExternalLink className="!size-4" /></a>
                        </span>
                    ) : (
                        <div className="font-bold truncate">{title || "(untitled)"}</div>
                    )}
                    <div className="text-xs text-muted-foreground flex items-center gap-2">
                        <Clock className="!size-3" /> Default {minutes}m
                    </div>
                    {(favorite as any).description && (<div className="text-xs text-muted-foreground truncate max-w-[340px]">{(favorite as any).description}</div>)}
                </div>
                <div className="flex items-center gap-2">
                    <Button onClick={() => setConfirmOpen(true)} size="sm" className="bg-accent bold flex items-center">Add Record <PlusCircle className="ml-2 !size-5 !stroke-2.5" /></Button>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="neutral" size="icon"><MoreHorizontal /></Button>
                        </DropdownMenuTrigger>
