...
type HeatmapProgressProps = React.ComponentProps<typeof ProgressPrimitive.Root> & {
    value?: number; // 0-100
};

export function HeatmapProgress({ className, value = 0, ...props }: HeatmapProgressProps) {
    ...
    return (
        <div className="relative scale-y-50">
            ...
            <div className={cn("absolute overflow-hidden -inset-[2.5px] rounded-full")}>
                <div className={cn("absolute inset-0 aspect-square -translate-y-[47%] scale-y-30", is100Percent && "bg-transparent", styles.heatmapSpin)} style={!is100Percent ? { backgroundColor: "var(--color-heatmap-bg)" } : {
                    backgroundImage: `linear-gradient(90deg,
                        var(--color-heatmap-4) 0%,
                        var(--color-heatmap-3) 25%,
                        var(--color-heatmap-2) 50%,
