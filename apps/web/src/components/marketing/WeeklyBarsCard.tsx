...
export function WeeklyBarsCard() {
    ...
    const weekly = useQuery(api.languageActivityFunctions.getWeeklySourceDistribution, {});
    const demoData = [
    ...
    const { prepared, maxTotal } = React.useMemo(() => {
        const withTotals = data.map((d) => {
            const total = (Number((d as any).youtube) || 0) + (Number((d as any).spotify) || 0) + (Number((d as any).anki) || 0) + (Number((d as any).misc) || 0);
    ...
    const PlaceholderShape = React.useCallback((props: any) => {
        ...
        return (
            <g pointerEvents="none">
                <rect x={x} y={y} width={width} height={height} rx={2} ry={2} fill="none" stroke="var(--color-border)" strokeDasharray="4 3" strokeWidth={2} />
            </g>
        );
    }, []);
    return (
        <Card>
            ...
            <CardContent>
                <ChartContainer config={chartConfig}>
                    <BarChart data={prepared} margin={{ top: 4, right: 8, left: 8, bottom: 16 }}>
                        ...
                        <ChartTooltip
                            cursor={false}
                            content={(props: any) => {
                                const payload = props?.payload?.filter((p: any) => p?.dataKey !== "__placeholder") ?? [];
                                return (
                                    <ChartTooltipContent
                                        {...props}
                                        payload={payload}
                                        labelKey="day"
                                        formatter={(value: number, name: string, item: any, index: number, payloadItem: any) => {
                                            const label = (chartConfig as any)[name]?.label || name;
                                            const color = (payloadItem?.fill ?? item?.payload?.fill ?? item?.color) as string | undefined;
                                            return (
                                                <>
