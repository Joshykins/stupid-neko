			...
			<CardContent>
				<ul className="grid grid-cols-1 sm:grid-cols-1 gap-2">
					{integrations.map((i) => (
						<li
							key={i.name}
							className="flex items-start gap-2 p-2 rounded-base transition-all border-2 border-border bg-secondary-background text-main-foreground border-border border-2 hover:translate-x-reverseBoxShadowX hover:translate-y-reverseBoxShadowY hover:shadow-shadow"
						>
							<img
								src={i.icon}
								alt={i.name}
							...
							<div className="flex flex-col">
								...
								<ul className="list-disc pl-4 mt-1 space-y-0.5">
									{i.points.map((p, idx) => (
										<li key={`${i.name}-${idx}`} className="text-xs">
											{p}
										</li>
									))}
								</ul>
