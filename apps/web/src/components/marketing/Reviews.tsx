...
const ReviewCard: React.FC<{ r: Review; } & React.HTMLAttributes<HTMLDivElement>> = ({ r, className = "", ...rest }) => (
    <Card className={"group relative w-[20rem] shrink-0 px-4 py-3 " + className} {...rest}>
        <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
                <div className="flex items-center gap-2">
                    <p className="truncate font-semibold text-main-foreground">{r.name}</p>
                    {r.handle && (
                        <span className="truncate text-xs text-foreground/70">@{r.handle}</span>
                    )}
                </div>
            ...
            {r.source && (
                ...
                <Badge className="ml-auto shrink-0 px-2 py-0.5 text-xs">{r.source}</Badge>
            )}
        </div>
        ...
        {r.date && (
            <p className="mt-2 text-xs text-foreground/70">{r.date}</p>
        )}
    </Card>
);
...
> = ({ reviews, direction = "left", speed = 80, className = "" }) => {
    ...
    return (
        <div
            className={
                "relative overflow-hidden rounded-base " +
                className
            }
        ...
        >
            ...
            <div className="relative flex">
                ...
                <div ref={containerRef} className="flex gap-4 will-change-transform px-2">
                    {duplicated.map((r, i) => (
...
> = ({ reviews, className = "" }) => {
    return (
        ...
        >
            ...
            <motion.div className="overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
                <motion.div
                    className="flex gap-4"
                    drag="x"
                    dragConstraints={{ left: -Math.max(0, reviews.length * 320 - 320), right: 0 }}
            ...
            <div className="mt-4 flex justify-center gap-1.5">
                {reviews.slice(0, 8).map((_, i) => (
                    <div key={i} className="h-1.5 w-6 rounded-full bg-main-foreground/20" />
                ))}
            </div>
        </div>
    );
...
const ReviewsShowcase: React.FC<ReviewsShowcaseProps> = ({ reviews, variant = "marquee", direction, speed, className }) => {
    if (variant === "card") {
        return <SwipeDeck reviews={reviews} className={className} />;
    }
    return <Marquee reviews={reviews} direction={direction} speed={speed} className={className} />;
};
...
export const demoReviews: Review[] = [
    {
        ...
        rating: 5,
        text:
            "Hit a 30‑day streak and the app dropped a celebratory meme in Discord. Actually made me laugh while studying kanji.",
        source: "Discord",
        date: "2 days ago",
    },
    {
        ...
        rating: 5,
        text:
            "The auto‑nya‑lytics are wild. I just listen to music and YouTube and the stats update by themselves.",
        source: "App",
        date: "This week",
    },
    {
    ...
    { id: "4", name: "Samir", rating: 5, text: "Clean UI, cute cat, and the XP bar is dangerously motivating.", source: "Twitter" },
    { id: "5", name: "Lucia", rating: 5, text: "I learn when I’m bored now. Subway Surfers + vocab = focus mode.", source: "App" },
    { id: "6", name: "Kenji", rating: 5, text: "Hit N5 in 6 months. The weekly charts showed exactly where I was slacking.", source: "App" },
];
