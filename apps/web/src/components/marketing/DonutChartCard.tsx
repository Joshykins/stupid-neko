...
export function DonutChartCard() {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle className="font-display text-xl font-black">Flashcards breakdown</CardTitle>
            </CardHeader>
            <CardContent>
                <div className="flex items-center gap-4">
                    <div className="relative aspect-square w-40 h-40">
                        <ChartContainer config={chartConfig} className="relative z-10 aspect-square w-full h-full">
                            <PieChart>
                                <Pie data={data} dataKey="value" nameKey="name" innerRadius={48} outerRadius={72} stroke="var(--color-border)" strokeWidth={2}>
                                ...
                                <ChartTooltip
                                    ...
                                    content={(props: any) => {
                                        ...
                                        const value = (item?.payload?.value ?? item?.value) as number | undefined;
                                        const color = (item?.payload?.fill ?? item?.color) as string | undefined;
                                        ...
                                        return (
                                            <div className="relative z-20 border-border bg-foreground grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs border-2 border-border shadow-shadow">
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <span className="text-main-foreground font-mono font-medium tabular-nums">{Number(value).toLocaleString()}</span>
                                                </div>
                                            </div>
                                        );
                                    }}
                        ...
                        <div className="pointer-events-none absolute inset-0 grid place-items-center z-0">
                            <div className="text-center leading-tight">
                                ...
                                <div className="text-xs font-bold text-muted-foreground">Flashcards</div>
                            </div>
                        </div>
                    </div>
                    <ul className="text-sm font-bold space-y-1">
                        {data.map((d) => (
                            <li key={d.name} className="flex items-center gap-2">
                                <span className="inline-block w-3 h-3 rounded-sm border-2 border-border" style={{ background: d.fill as React.CSSProperties["color"] }} />
                                {d.value} {d.name}
                            </li>
                        ))}
                    </ul>
