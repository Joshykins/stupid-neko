...
export default function StreakDisplayWeek({ title, days, values, onHover, intensityStyle, totalDays, liveVersion = false, activityCounts, vacationFlags }: Props) {
    ...
    const items: Array<{ absoluteIndex: number; intensity: number; minutes: number; isVacation: boolean; }> = [];
    for (let i = 0; i < daysToShow; i++) {
        const absoluteIndex = weekStart + i;
        const intensity = renderValues[absoluteIndex] ?? 0;
        const minutes = liveVersion
            ? (streakData && streakData.activityCounts && streakData.activityCounts.length === effectiveTotalDays ? (streakData.activityCounts[absoluteIndex] ?? 0) : (intensity > 0 ? 1 : 0))
            : (activityCounts && activityCounts.length === effectiveTotalDays ? (activityCounts[absoluteIndex] ?? 0) : (intensity > 0 ? 1 : 0));
    ...
    const startDate = new Date(today.getTime() - (effectiveTotalDays - 1) * 24 * 60 * 60 * 1000);

    return (
        <div className="w-full p-3">
            <div className="grid grid-cols-7 gap-2 sm:gap-3">
                {items.map((it, i) => {
                    const seed = hashStringToSeed(`${title}-week-${effectiveTotalDays}-${i}`);
                    const rng = createSeededRng(seed);
                    const delay = 0.1 + rng() * 0.6;
                    const hasActivity = it.minutes > 0 || it.intensity > 0 || it.isVacation;
                    ...
                    const gradientOverlay: React.CSSProperties = hasActivity && !isVacation
                        ? { backgroundImage: "linear-gradient(180deg, rgba(255,255,255,0.35), rgba(0,0,0,0.06))" }
                        : {};
                    ...
                    return (
                        <div key={i} className="flex flex-col items-center justify-start gap-1">
                            <div
                            ...
                            >
                                ...
                                <motion.div
                                    ...
                                    style={{ width: size, height: size, ...baseStyle, ...gradientOverlay }}
