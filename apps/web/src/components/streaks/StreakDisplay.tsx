...
export default function StreakDisplay({ title = "Daily Streak", days = 365, values, cellSize = 12, liveVersion = false }: Props) {
    const streakData = useQuery(api.streakFunctions.getStreakDataForHeatmap, liveVersion ? { days } : "skip");
    const me = useQuery(api.meFunctions.me, liveVersion ? {} : "skip");
    const updateMode = useMutation(api.meFunctions.updateStreakDisplayMode);
    ...
    const mock = React.useMemo(() => {
        ...
        for (let i = 0; i < total; i++) {
            const r = rng();
            if (r < 0.02) vals[i] = 0; // very rare zeros
            else if (r < 0.07) vals[i] = 1;
            else if (r < 0.20) vals[i] = 2;
        ...
        const targetStreak = Math.min(total, Math.floor(minStreak + rng() * (maxStreak - minStreak + 1)));
        ...
        while (placed < groupCount && guard < 200) {
            ...
            const start = startMin + Math.floor(rng() * Math.max(1, startMax - startMin + 1));
            ...
            for (let i = start - 2; i < start + len + 2; i++) {
                if (i >= 0 && i < total && used.has(i)) { ok = false; break; }
            }
            if (!ok) continue;
            for (let i = 0; i < len; i++) {
        ...
        for (let i = 0; i < total; i++) {
            if (vacationFlags[i]) { minutes[i] = 0; continue; }
            const v = vals[i];
            let base = 0;
        ...
        for (let i = total - 1; i >= 0; i--) {
            ...
            if (credited) currentStreak++; else break;
        }
        // If below target due to placement randomness, force it
        if (currentStreak < targetStreak) {
            const need = targetStreak - currentStreak;
            for (let i = total - currentStreak - 1; i >= total - currentStreak - need && i >= 0; i--) {
    ...
    const startDate = React.useMemo(() => {
        const d = new Date();
        d.setHours(0, 0, 0, 0);
        d.setDate(d.getDate() - (((streakData?.totalDays ?? mock?.totalDays) ?? days) - 1));
        return d;
    ...
    const saveMode = React.useCallback(async (next: "grid" | "week") => {
        setMode(next);
        if (!liveVersion) return;
        if (!me) return;
        try { await updateMode({ mode: next }); } catch { }
    ...
    const intensityStyle = (n: number): React.CSSProperties => {
        ...
        return { backgroundColor: colors[Math.max(0, Math.min(HEATMAP_INTENSITY_STEPS - 1, n))] };
    };

    const [tooltip, setTooltip] = React.useState<{ visible: boolean; x: number; y: number; label: string; }>({ visible: false, x: 0, y: 0, label: "" });
    ...
    React.useLayoutEffect(() => {
        if (tooltipRef.current) setTooltipSize({ w: tooltipRef.current.offsetWidth, h: tooltipRef.current.offsetHeight });
    }, [tooltip.visible, tooltip.label]);

    const totalDaysEffective = (streakData?.totalDays ?? mock?.totalDays) ?? days;
    const activityCounts = (streakData?.activityCounts ?? mock?.activityCounts) as number[] | undefined;
    const vacationFlags = (streakData?.vacationFlags ?? mock?.vacationFlags) as boolean[] | undefined;
    ...
    const onHover = (absoluteIndex: number, value: number) => {
        ...
        const minutesLearned = activityCounts && activityCounts.length === totalDaysEffective
        ...
        const timePart = minutesLearned > 0
            ? ` • ${hours}:${mins.toString().padStart(2, "0")}`
            : "";

        const isVacation = Boolean(vacationFlags && vacationFlags.length === totalDaysEffective && vacationFlags[absoluteIndex]);
    ...
    const onMove = (ev: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (!rect) return;
        setTooltip((t) => (t.visible ? { ...t, x: ev.clientX - rect.left + 12, y: ev.clientY - rect.top + 12 } : t));
    };
    const onLeave = () => setTooltip({ visible: false, x: 0, y: 0, label: "" });

    // Use real or mock current streak and bonus percent
    const currentStreak = liveVersion ? (streakData?.currentStreak ?? 0) : (mock?.currentStreak ?? 0);
    ...
    const effectiveValues = liveVersion
        ? (streakData && streakData.values.length === (days ?? 0) ? streakData.values : values)
        : (mock?.values ?? values);
    ...
    return (
        <Card>
            ...
            <CardContent className="px-4">
                <div ref={containerRef} className={`w-full rounded-md overflow-hidden border-2 border-black ${mode === "grid" ? "bg-heatmap-bg" : "bg-foreground"} relative`} onMouseMove={onMove} onMouseLeave={onLeave}>
