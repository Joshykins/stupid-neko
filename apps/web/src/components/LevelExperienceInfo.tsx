...
export default function LevelExperienceInfo({ maxLevel = 60 }: { maxLevel?: number; }) {
    ...
    const cutoffLevel = React.useMemo(() => detectCostCutoff(Math.max(120, maxLevel)), [maxLevel]);
    // Bucket into ranges of 10 levels by default
    const bucketSize = 10;
    const bucketedData = React.useMemo(() => bucketizeLevels(bucketSize, Math.min(maxLevel, cutoffLevel)), [bucketSize, maxLevel, cutoffLevel]);
    const flatCostAfterCutoff = React.useMemo(() => xpForNextLevel(cutoffLevel), [cutoffLevel]);
    ...
    return (
        <Popover>
            <PopoverTrigger asChild>
                <Button size="icon" variant={"neutral"} className="h-7 w-7 p-0 flex items-center justify-center" aria-label="Level/XP info">
                    <Info className="stroke-3 text-background !size-5" />
                </Button>
            </PopoverTrigger>
            <PopoverContent className="w-96 p-3 bg-secondary-background">
                <div className="mb-2">
                    <div className="font-semibold leading-none text-background">Level & Experience Curve</div>
                    <div className="text-xs text-background mt-1">
                        Each level requires more XP until level {cutoffLevel}. After that, each level costs {flatCostAfterCutoff.toLocaleString()} XP.
                    </div>
                </div>
                <div className="pr-2">
                    <ChartContainer
                        config={{
                            cost: { label: "XP to Next Level", color: "var(--color-heatmap-1)" },
                        }}
                        className="aspect-[4/3] rounded-base border-2 border-border px-2 pt-2"
                    >
                        <AreaChart
                            className="!text-background" data={dataWithTail} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis
                                className="!fill-background"
                                dataKey={(d: BucketPoint) => d.isFlatTail ? `${cutoffLevel}+` : `${d.rangeStart}-${d.rangeEnd}`}
                                tickLine={false}
                                axisLine={false}
                                tickMargin={8}
                            ...
                            <ChartTooltip content={((props: any) => (
                                <ChartTooltipContent
                                    {...props}
                                    labelFormatter={() => ""}
                                    formatter={(_value: number, _name: string, _item: any, _index: number, payload: BucketPoint) => {
                                        return (
                                            <div className="flex w-full flex-col gap-0.5">
                                                <div className="font-medium">{payload.isFlatTail ? `Levels ${cutoffLevel}+` : `Levels ${payload.rangeStart}-${payload.rangeEnd}`}</div>
                                                <div className="text-muted-foreground">Avg cost: {Math.round(payload.avgDeltaXp).toLocaleString()}</div>
                                                <div className="text-muted-foreground">Range: {Math.round(payload.minDeltaXp).toLocaleString()} – {Math.round(payload.maxDeltaXp).toLocaleString()}</div>
                                            </div>
                                        );
                                    }}
                                />
                            )) as any} />
                            <Area type="monotone" dataKey="avgDeltaXp" name="cost" fill="var(--color-cost)" stroke="var(--color-cost)" />

                        </AreaChart>
                    </ChartContainer>
                    <div className="text-xs text-background mt-2">
                        100xp roughly equalts 1 hour of study. With streak bonus it equals 30 minutes. Leagues, quests, and more also award XP.
                    </div>
                </div>
            </PopoverContent>
        </Popover>
