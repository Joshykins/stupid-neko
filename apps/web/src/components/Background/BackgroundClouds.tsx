...
function Cloud({ topVh, scale, speedSec, delaySec, opacity, fadeDurationSec, fadeDelaySec, zIndex }: CloudInstance) {
        ...
        <div className="absolute" style={outerStyle}>
            <div style={fadeStyle}>
                <div style={travelStyle}>
                    <div className={cx("relative overflow-hidden blur-xs")} style={cloudStyle}>
                        {/* Circles composing the cloud silhouette (port from SCSS) */}
                        <div className={cx(circleBase)} style={{ width: 32, height: 32, left: 0, bottom: -15, background: CLOUD_COLOR }} />
                        <div className={cx(circleBase)} style={{ width: 35, height: 35, left: 20, bottom: 0, background: CLOUD_COLOR }} />
                        <div className={cx(circleBase)} style={{ width: 25, height: 25, left: 48, bottom: 15, background: CLOUD_COLOR }} />
                        <div className={cx(circleBase)} style={{ width: 35, height: 35, left: 65, bottom: 20, background: CLOUD_COLOR }} />
                        <div className={cx(circleBase)} style={{ width: 25, height: 25, left: 94, bottom: 16, background: CLOUD_COLOR }} />
                        <div className={cx(circleBase)} style={{ width: 30, height: 30, left: 110, bottom: -5, background: CLOUD_COLOR }} />
                        <div className={cx(circleBase)} style={{ width: 30, height: 30, left: 132, bottom: -15, background: CLOUD_COLOR }} />
                        <div className={cx(circleBase)} style={{ width: 90, height: 90, left: 30, bottom: -55, background: CLOUD_COLOR }} />
                    </div>
                </div>
            </div>
        </div>
...
export const BackgroundClouds: FC<{ className?: string; enabled?: boolean; }> = ({ className, enabled = true }) => {
    ...
    useEffect(() => {
        ...
        const totalCount = randInt(CLOUD_COUNT_RANGE[0], CLOUD_COUNT_RANGE[1] + 0.49);

        const built: Array<CloudInstance> = [];
        const opacityThreshold = (CLOUD_OPACITY_RANGE[0] + CLOUD_OPACITY_RANGE[1]) / 2;

        for (let i = 0; i < totalCount; i += 1) {
            const speedSec = rand(CLOUD_SPEED_SEC_RANGE[0], CLOUD_SPEED_SEC_RANGE[1]);
            const baseDelay = rand(CLOUD_DELAY_SEC_RANGE[0], CLOUD_DELAY_SEC_RANGE[1]);
            ...
            built.push({
                ...
                opacity,
                fadeDurationSec: rand(CLOUD_FADE_DURATION_SEC_RANGE[0], CLOUD_FADE_DURATION_SEC_RANGE[1]),
                fadeDelaySec: rand(CLOUD_FADE_DELAY_SEC_RANGE[0], CLOUD_FADE_DELAY_SEC_RANGE[1]),
    ...
    return (
        <div className={cx("absolute inset-x-0 top-0 pointer-events-none select-none", className)} style={{ height: `${CLOUD_SPAWN_AREA_VH}vh` }}>
            <style>{`
