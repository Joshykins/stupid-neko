...
const demoPoints = (days: number): Array<{ name: string; xp: number; }> => {
    ...
    for (let i = days - 1; i >= 0; i--) {
        ...
        const minutes = Math.max(0, Math.round((baseMinutes + bonus + restDrop) * weeklyMultiplier[(dayOfWeek + 6) % 7] * trend));
...
export default function XpAreaChart({ isLiveVersion = false }: XpAreaChartProps) {
    const [range, setRange] = React.useState<Range>("7d");
    const data = useQuery(api.xpChartFunctions.getXpTimeseries, isLiveVersion ? { range } as { range: Range; } : "skip");
    const gradientRawId = React.useId();
    const gradientId = React.useMemo(() => `xpGradient_${gradientRawId.replace(/:/g, "")}`, [gradientRawId]);
    ...
    return (
        <Card>
            ...
            <CardHeader className="flex items-start justify-between pb-4">
                <CardTitle className="font-display text-xl font-black flex flex-col gap-2">Experience Trend
                ...
                <div className="inline-flex gap-2">
                    {(["7d", "30d", "all"] as Range[]).map((r) => (
                        <Button key={r} size="sm" type="button" variant={range === r ? "default" : "ghost"} onClick={() => setRange(r)}>
                            {r}
                        </Button>
                    ))}
                </div>
            ...
            <CardContent>
                ...
                >
                    <AreaChart data={chartData} margin={{ left: 8, right: 8, top: 8, bottom: 8 }} className="!text-background">
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis className="!fill-background" dataKey="name" tickLine={false} axisLine={false} minTickGap={24} />
                        <YAxis className="!fill-background" tickLine={false} axisLine={false} width={40} />
                        <ChartTooltip
                            content={((props: any) => (
                                <ChartTooltipContent
                                    {...props}
                        ...
                        />
                        <Area type="monotone" dataKey="xp" name="xp" fill={stroke} stroke={stroke} fillOpacity={0.25} strokeWidth={2} />
                    </AreaChart>
                </ChartContainer>
            </CardContent>
        </Card>
