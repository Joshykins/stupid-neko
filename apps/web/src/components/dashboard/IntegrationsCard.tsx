...
export default function IntegrationsCard() {
	...
	const isBrowserExtensionConnected = React.useMemo(() => {
		...
		return !!(
			...
			integrationKey?.integrationKeyUsedByPlugin
		);
	}, [
		integrationKey?.integrationId,
		integrationKey?.integrationKeyUsedByPlugin,
		...
		<Card>
			...
			<CardContent className="space-y-4">
				{items.map(i => (
					<div
						key={i.id}
						className="flex items-center justify-between gap-4 p-3 rounded-lg border border-border/50 hover:border-border transition-colors"
					>
							...
							<div className="space-y-3">
								...
								<div className="p-4 rounded-lg bg-muted/50 border">
									<p className="text-sm text-muted-foreground">
										...
										establish the connection. The toggle will automatically
										enable once connected.
									</p>
								</div>
							</div>
						</div>
					...
					<DialogContent className="max-w-2xl">
						...
						<div className="space-y-6">
							...
							<div className="space-y-3">
								...
								<div className="p-4 rounded-lg bg-muted/50 border">
									<p className="text-sm text-muted-foreground mb-3">
										This is your unique integration key that connects your
										browser extension to your account.
									</p>
									<div className="relative w-full">
										<Input
											readOnly
