import { action, httpAction, internalMutation, mutation, query, internalQuery } from "./_generated/server";
...
export const getStatus = query({
	args: {},
	returns: v.object({ connected: v.boolean(), displayName: v.optional(v.string()) }),
	handler: async (ctx) => {
...
export const updateAccessToken = internalMutation({
    args: { id: v.id("spotifyAccounts"), accessToken: v.string(), expiresAt: v.number() },
    returns: v.null(),
    handler: async (ctx, args) => {
        await ctx.db.patch(args.id, { accessToken: args.accessToken, expiresAt: args.expiresAt, updatedAt: Date.now() } as any);
        return null;
    },
});
