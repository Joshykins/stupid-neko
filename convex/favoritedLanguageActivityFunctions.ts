...
export const createFavorite = mutation({
    ...
    returns: v.object({ favoriteId: v.id("favoritedLanguageActivities") }),
    handler: async (ctx, args) => {
        const userId = await getAuthUserId(ctx);
        if (!userId) throw new Error("Unauthorized");
        const user = await ctx.db.get(userId);
        const utlId = (user as any)?.currentTargetLanguageId as Id<"userTargetLanguages"> | undefined;
        if (!utlId) throw new Error("No active target language selected");

        const favoriteId = await ctx.db.insert("favoritedLanguageActivities", {
            userId,
            userTargetLanguageId: utlId,
            title: args.title,
            description: args.description ?? undefined,
            externalUrl: args.externalUrl ?? undefined,
            defaultDurationInMinutes: Math.max(0, Math.round(args.defaultDurationInMinutes ?? 10)),
...
export const listFavorites = query({
    ...
    returns: v.array(
        v.object({
            _id: v.id("favoritedLanguageActivities"),
            ...
            externalUrl: v.optional(v.string()),
            defaultDurationInMinutes: v.optional(v.number()),
            createdFromLanguageActivityId: v.optional(v.id("languageActivities")),
    ...
    handler: async (ctx) => {
        ...
        const favorites = await ctx.db
            .query("favoritedLanguageActivities")
            .withIndex("by_user_target_language", (q: any) => q.eq("userTargetLanguageId", utlId))
            .order("desc")
            .collect();
...
export const listManualActivitiesWithFavoriteMatch = query({
    args: { limit: v.optional(v.number()), cursorOccurredAt: v.optional(v.number()) },
    returns: v.object({
        page: v.array(
            v.object({
                ...
                matchedFavoriteId: v.optional(v.id("favoritedLanguageActivities")),
            }),
        ),
        isDone: v.boolean(),
    ...
    handler: async (ctx, args) => {
        ...
        const continueCursor = page.length > 0 ? (page[page.length - 1] as any).occurredAt : undefined;
        return { page, isDone, continueCursor } as any;
    },
});

export const addFavoriteFromActivity = mutation({
    args: { activityId: v.id("languageActivities"), isFavorite: v.boolean() },
    returns: v.object({ favoriteId: v.optional(v.id("favoritedLanguageActivities")), isFavorite: v.boolean() }),
    handler: async (ctx, args) => {
        const userId = await getAuthUserId(ctx);
        if (!userId) throw new Error("Unauthorized");
        ...
        if (!(act as any).isManuallyTracked) throw new Error("Only manual activities can be favorited");
        ...
        const existing = await ctx.db
            ...
            .withIndex("by_user_target_language", (q: any) => q.eq("userTargetLanguageId", utlId))
            .collect();
        const match = existing.find((f: any) => (f.title ?? "").trim().toLowerCase() === title.toLowerCase());

        if (args.isFavorite) {
            const favoriteId = match?._id as Id<"favoritedLanguageActivities"> | undefined ?? await ctx.db.insert("favoritedLanguageActivities", {
                userId,
                userTargetLanguageId: utlId,
                title: (act as any).title ?? "",
                description: (act as any).description ?? undefined,
                externalUrl: (act as any).externalUrl ?? undefined,
                defaultDurationInMinutes: Math.max(0, Math.round(((act as any).durationInSeconds ?? 0) / 60)),
...
export const quickCreateFromFavorite = mutation({
    args: {
        favoriteId: v.id("favoritedLanguageActivities"),
    ...
    handler: async (ctx, args): Promise<{ activityId: Id<"languageActivities"> }> => {
        ...
        const utlId = (fav as any).userTargetLanguageId as Id<"userTargetLanguages">;
        const now = await getEffectiveNow(ctx);
        const occurredAt = args.occurredAt ?? now;

        const durationInMinutes = Math.max(0, Math.round(args.durationInMinutes ?? (fav as any).defaultDurationInMinutes ?? 10));

        const user = await ctx.db.get(userId);
        const utl = await ctx.db.get(utlId);
        const languageCode = (utl as any)?.languageCode;

        const created = await ctx.runMutation(internal.languageActivityFunctions.addLanguageActivity, {
            userTargetLanguageId: utlId,
        ...
        const activityId = (created as { activityId: Id<"languageActivities"> }).activityId;
        return { activityId };
    },
});
...
export const updateFavorite = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (typeof args.externalUrl !== "undefined") patch.externalUrl = args.externalUrl;
        if (typeof args.defaultDurationInMinutes !== "undefined") patch.defaultDurationInMinutes = Math.max(0, Math.round(args.defaultDurationInMinutes));
...
export const deleteFavorite = mutation({
    args: { favoriteId: v.id("favoritedLanguageActivities") },
